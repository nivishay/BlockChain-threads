...
void Server::start(BLOCK_T& block){
    block = {1, static_cast<int>(time(nullptr)), 0, 0, DIFFICULTY, 0, -1}; // TODO: CHANGE PRIORTY TO BE HIGHT THEN MINERS
    while (true) {
        ...
        addBlock(block);
        sleep(2);
        block = {block.height + 1,static_cast<int>(time(nullptr)), 0,blockChain.back().hash, DIFFICULTY,1, -1};
